// IslEngine.cs — pragmatic C# parser for the documented ISL v2.0 subset.
// Matches JS grammar for: intent <verb>("...") in world("...") [at "..."] [with evidence("...")|params {...}];
// Authoritative runtime for browser: engine/scripting/IslInterpreter.js + IslParser.js.
// Status: host mirror — CompileAndEvaluate for play_timeline / render_4d_tesseract forms.
    ...
    public sealed class IslEngine : IIslEngine
    {
        ...
        static Dictionary<string, object> ParseContext(string contextJson)
        {
            ...
            if (string.IsNullOrWhiteSpace(contextJson) || contextJson.Trim() == "{}")
                return ctx;
            // Minimal JSON string field extraction (actor, worldId) — not a full JSON parser.
