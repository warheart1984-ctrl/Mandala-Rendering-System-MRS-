...
#include "FourDRendererTypes.generated.h"

/**
 * FourDRenderer v2.0 host contract types (Unreal).
 * Field names match docs/4d-engine/v2 RFCs.
 * Status: skeleton — structs + live-link hooks only; does not claim 4D path tracing / RHI.
 *
 * Layout note: do NOT redefine UE's FVector4 / FVector. Use FFourDVector4 for Blueprint
 * USTRUCTs, and FVector4f / FVector3f for plain C++ mirrors when needed.
 */
...
/** Plain (non-USTRUCT) mirrors for CPU / future compute — use engine float vectors. */
struct FRay4D
{
