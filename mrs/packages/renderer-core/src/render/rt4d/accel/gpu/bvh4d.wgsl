// Production WebGPU/WGSL 4D BVH traversal + intersection shaders.
// Designed for compute pass dispatch: 1 thread per ray.
//
// Bind group layout:
...
const STACK_SIZE: i32 = 64;
const EPS: f32 = 1e-12;

// ─── Safe denominator ───────────────────────────────────────────
fn safeDenom(d: f32) -> f32 {
    if (abs(d) > EPS) { return d; }
    return select(-EPS, EPS, d >= 0.0);
}
...
fn intersectAABB4D(o: vec4<f32>, d: vec4<f32>, bmin: vec4<f32>, bmax: vec4<f32>) -> vec2<f32> {
    ...
    for (var k: i32 = 0; k < 4; k++) {
        let ok = o[k];
        let dk = d[k];
        let invD = 1.0 / safeDenom(dk);
        let t1 = (bmin[k] - ok) * invD;
        let t2 = (bmax[k] - ok) * invD;
...
fn intersectHypersphere(o: vec4<f32>, d: vec4<f32>, sph: Hypersphere4D,
                        tMin: f32, tMax: f32) -> vec2<f32> {
    let oc = o - sph.center;
    let a = dot(d, d);
    let b = 2.0 * dot(oc, d);
...
fn hypersphereNormal(p: vec4<f32>, c: vec4<f32>) -> vec4<f32> {
    let n = p - c;
    return normalize(n);
}

// ─── Hyperplane intersection ────────────────────────────────────
fn intersectHyperplane(o: vec4<f32>, d: vec4<f32>, plane: Hyperplane4D,
                       tMin: f32, tMax: f32) -> vec2<f32> {
    let nd = dot(plane.normal, d);
    if (abs(nd) < EPS) { return vec2<f32>(1e30, -1e30); }
    let t = (plane.offset - dot(plane.normal, o)) / nd;
    if (t < tMin || t > tMax) { return vec2<f32>(1e30, -1e30); }
    return vec2<f32>(t, 0.0);
}

// ─── Mesh triangle intersection (4D wedge-product Möller–Trumbore)
fn intersectMeshTri(o: vec4<f32>, d: vec4<f32>, tri: MeshTri4D,
                    tMin: f32, tMax: f32) -> vec2<f32> {
    let e1 = tri.v1 - tri.v0;
    let e2 = tri.v2 - tri.v0;

    // 4D cross product (bivector → vector)
    let pvec = vec4<f32>(
        d.y * e2.z - d.z * e2.y,
        d.z * e2.w - d.w * e2.z,
        d.w * e2.x - d.x * e2.w,
    ...
    let det = dot(e1, pvec);
    if (abs(det) < EPS) { return vec2<f32>(1e30, -1e30); }

    let invDet = 1.0 / det;
    let tvec = o - tri.v0;

    let u = dot(tvec, pvec) * invDet;
    if (u < 0.0 || u > 1.0) { return vec2<f32>(1e30, -1e30); }

    let qvec = vec4<f32>(
        tvec.y * e1.z - tvec.z * e1.y,
        tvec.z * e1.w - tvec.w * e1.z,
        tvec.w * e1.x - tvec.x * e1.w,
        tvec.x * e1.y - tvec.y * e1.x
    );
    let v = dot(d, qvec) * invDet;
    if (v < 0.0 || u + v > 1.0) { return vec2<f32>(1e30, -1e30); }

    let t = dot(e2, qvec) * invDet;
    if (t < tMin || t > tMax) { return vec2<f32>(1e30, -1e30); }
    return vec2<f32>(t, 1.0);  // second component = 1 signals mesh tri hit
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let o = rayOrigins[idx];
    let d = rayDirs[idx];
    let tMin = rayTMin[idx];
    let tMax = rayTMax[idx];
    ...
    var closestT: f32 = tMax;
    var closestPrim: i32 = -1;
    var closestMat: i32 = -1;
    var closestN: vec4<f32> = vec4<f32>(0.0);
    ...
    loop {
        ...
        if (tRange.x > tRange.y || tRange.x > closestT) {
            continue;
        }

        if (node.primCount > 0) {
            for (var i: i32 = 0; i < node.primCount; i = i + 1) {
                let primId = node.firstPrim + i;
                let type = primType[primId];
                let offset = primOffset[primId];

                var tHit: f32 = 1e30;
                var hit = false;

                if (type == 0) {  // Hypersphere
                    let r = intersectHypersphere(o, d, spheres[offset], tMin, closestT);
                    if (r.x < r.y && r.x < closestT) {
                        tHit = r.x;
                        let p = o + tHit * d;
                        closestN = hypersphereNormal(p, spheres[offset].center);
                        closestMat = spheres[offset].materialId;
                        hit = true;
                    }
                } else if (type == 1) {  // Hyperplane
                    let r = intersectHyperplane(o, d, planes[offset], tMin, closestT);
                    if (r.x < r.y && r.x < closestT) {
                        tHit = r.x;
                        closestN = planes[offset].normal;
                        closestMat = planes[offset].materialId;
                        hit = true;
                    }
                } else if (type == 2) {  // Mesh triangle
                    let r = intersectMeshTri(o, d, meshTris[offset], tMin, closestT);
                    if (r.x < r.y && r.x < closestT) {
                        tHit = r.x;
                        closestN = meshTriNormal(meshTris[offset], 0.0, 0.0);
                        closestMat = meshTris[offset].materialId;
                        hit = true;
                    }
                }
            ...
            if (node.leftChild >= 0 && sp < 63) {
                stack[sp] = node.leftChild;
                sp = sp + 1;
            }
            if (node.rightChild >= 0 && sp < 63) {
